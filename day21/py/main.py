...
# Run a list of springcode instructions
def run_springcode(memory: Memory, springcode: List[str]) -> Optional[int]:
    _, _, output = execute_program(memory.copy(), [ord(char) for char in '\n'.join(springcode) + '\n'])
    for char in output:
        if char > 128:
            return char
        else:
